#Notice:
#This file is created by DeepSeek AI, not me.
#Website of DeepSeek: https://chat.deepseek.com/
...
class Enc(Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size, maximum_position_encoding, rate=0.1):
        ...
        self.enc_layers = [EncLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
...
class Dec(Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, target_vocab_size, maximum_position_encoding, rate=0.1):
        ...
        self.dec_layers = [DecLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inp, tar, training, enc_padding_mask, look_ahead_mask, dec_padding_mask):
        enc_output = self.Enc(inp, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)

        # dec_output.shape == (batch_size, tar_seq_len, d_model)
        dec_output, attention_weights = self.Dec(tar, enc_output, training, look_ahead_mask, dec_padding_mask)
